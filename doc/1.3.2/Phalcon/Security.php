...
namespace Phalcon
{
	 ...
	 * Phalcon\Security
	 *
	 * This component provides a set of functions to improve the security in
	 * Phalcon applications
	 *
	 * <code>
	...
	class Security implements \Phalcon\DI\InjectionAwareInterface
	{
		...
		/**
		 * Returns a number of bytes to be generated by the openssl pseudo
		 * random generator
		 *
		 * @return string
		 */
		...
		/**
		 * Generates a pseudo random token key to be used as input's name in a
		 * CSRF check
		 *
		...
		/**
		 * Generates a pseudo random token value to be used as input's value in
		 * a CSRF check
		 *
		 * @param int $numberBytes
		 * @return string
		...
		/**
		 * Check if the CSRF token sent in the request is the same that the
		 * current in session
		 *
		...
		/**
		 * string \Phalcon\Security::computeHmac(string $data, string $key,
		 * string $algo, bool $raw = false)
		 */
		 ...
		 * for the derivation, by default 5000
		 * @param int $size The length of the output string. If 0 is passed (the
		 * default), the entire output of the supplied hash algorithm is used
		...
		/**
		 *
		 * @internal @brief This method is used only for internal tests, use
		 * \Phalcon\Security::deriveKey() instead
		 */
