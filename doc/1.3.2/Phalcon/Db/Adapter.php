...
namespace Phalcon\Db
{
	...
	abstract class Adapter implements \Phalcon\Events\EventsAwareInterface, \Phalcon\Db\AdapterInterface
	{
		 ...
		 * $robots = $connection->fetchAll("SELECT * FROM robots WHERE name LIKE
		 * :name",
		 * Phalcon\Db::FETCH_ASSOC,
		 * array('name' => '%robot%')
		 * );
		...
		/**
		 * Returns the default identity value to be inserted in an identity
		 * column
		 *
		 * <code>
		 * //Inserting a new robot with a valid default value for the column
		 * 'id'
		 * $success = $connection->insert(
		 * "robots",
