...
namespace Phalcon\Events
{
	 ...
	 *
	 * Phalcon Events Manager, offers an easy way to intercept and manipulate,
	 * if needed,
	 * the normal flow of operation. With the EventsManager the developer can
	 * create hooks or
	 * plugins that will offer monitoring of data, manipulation, conditional
	...
	class Manager implements \Phalcon\Events\ManagerInterface
	{
		...
		/**
		 * Tells the event manager if it needs to collect all the responses
		 * returned by every
		...
		/**
		 * Check if the events manager is collecting all all the responses
		 * returned by every
		 * registered listener in a single fire
		 */
		...
		/**
		 * Returns all the responses returned by every handler executed by the
		 * last 'fire' executed
		 *
		 * @return array
		 */
		...
		/**
		 * Fires an event in the events manager causing that active listeners be
		 * notified about it
		 *
		 * <code>
