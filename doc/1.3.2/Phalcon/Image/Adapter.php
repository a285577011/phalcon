...
namespace Phalcon\Image
{
	...
	abstract class Adapter implements \Phalcon\Image\AdapterInterface
	{
		 ...
		 * Either the width or the height can
		 * be omitted and the image will be resized proportionally.
		 *
		 * @param int $width new width
		 * @param int $height new height
		 * @param int $master master dimension, if $master is TENSILE, the width
		 * and height must be specified
		 ...
		 * Only support Imagick
		 *
		 * @param int $width new width
		 * @param int $height new height
		 * @param int $delta_x How much the seam can traverse on x-axis. Passing
		 ...
		 * Either the width or the height can be
		 * omitted and the current width or height will be used.
		 *
		 * @param int $width new width
		 * @param int $height new height
		 * @param int $offset_x offset from the left, if it's true then will
		 * center
		 * @param int $offset_y offset from the top, if it's true then will
		 * middle
		 * @return \Phalcon\Image\Adapter
		 */
		 ...
		 * The most opaque part of the reflection
		 * will be equal to the opacity setting and fade out to full
		 * transparent.
		 ...
		 * @param int $offset_x offset from the left, If less than 0 offset from
		 * the right, If true right the x offset
		 * @param int $offset_y offset from the top, If less than 0 offset from
		 * the bottom, If true bottom the Y offset
		...
		/**
		 * Save the image.
		 * If the filename is omitted, the original image will
		 * be overwritten.
		 *
