...
function Advertisement(option) {
	...
	function exportHtml(html, type) {
		...
		if (parseInt(type) == 2) {
			...
			else if(document.getElementById('tuiguang_box')){
			domain_lunbo(document.getElementById('tuiguang_box'));
		       }
			else if(document.getElementById('tuiguang_dianpu')){
				domain_lunbo(document.getElementById('tuiguang_dianpu'));
			}
		}
	}
	function beforeSend() {
...
function domain_lunbo(obj){
	console.log(obj);
	var domain_item_num = obj.getElementsByTagName('li').length;
	var _width = obj.getElementsByClassName('tuiguang_ul_box')[0].offsetWidth;
	var domain_item_width = _width/2;
	var charge_item = 2;
	for (var i = domain_item_num-1; i >= 0; i--) {
		obj.getElementsByTagName('li')[i].style.width = domain_item_width + 'px';
	};
	var domain_ul = obj.getElementsByClassName('tuiguang_ul')[0];
	obj.getElementsByClassName('tuiguang_ul')[0].style.width = domain_item_width * domain_item_num + 'px';

	var domain_ul_width = domain_item_width * domain_item_num;

	var prebtn = obj.getElementsByClassName('pre_btn');
	for (var i = prebtn.length - 1; i >= 0; i--) {
		prebtn[i].addEventListener('touchstart', function (e){
			var marleft = parseInt(domain_ul.style.marginLeft) || 0;
			if(marleft >= 0) return;
			if(marleft + charge_item * domain_item_width >= 0) {
				domain_ul.style.marginLeft = '0px';
			}else{
				domain_ul.style.marginLeft = marleft + charge_item * domain_item_width + 'px';
			}
		},false)
	};
	var nextbtn = obj.getElementsByClassName('next_btn');
	for (var i = nextbtn.length - 1; i >= 0; i--) {
		nextbtn[i].addEventListener('touchstart', function (e){
			var marleft = parseInt(domain_ul.style.marginLeft) || 0;
			if(marleft <= -domain_ul_width + _width ) return;
			if(marleft - charge_item * domain_item_width <= - domain_ul_width + charge_item * domain_item_width ) {
				domain_ul.style.marginLeft = - domain_ul_width + charge_item * domain_item_width + 'px';
			}else{
				domain_ul.style.marginLeft = marleft - charge_item * domain_item_width + 'px';
			}
