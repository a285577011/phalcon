...
abstract class FunctionalTestCase extends ModelTestCase
{
	...
	/**
	 * Assert that the last dispatched controller matches the given controller
	 * class name
	 *
	...
	{
		$actual = $this->di->getShared('dispatcher')
			->getControllerName();
		if($actual != $expected)
		{
	...
	public function assertHeader(array $expected)
	{
		...
		{
			$actualValue = $this->di->getShared('response')
				->getHeaders()
				->get($expectedField);
			if($actualValue != $expectedValue)
			{
				throw new \PHPUnit_Framework_ExpectationFailedException(
					sprintf('Failed asserting "%s" has a value of "%s", actual "%s" header value is "%s"', 
						$expectedField, $expectedValue, $expectedField, $actualValue));
			}
	...
	public function assertResponseCode($expected)
	{
		...
		{
			throw new \PHPUnit_Framework_ExpectationFailedException(
				sprintf('Failed asserting response code "%s", actual response code is "%s"', $expected, $actualValue));
		}
