 ...
 * @copyright Copyright (c) 2013 meets-ecommerce GmbH
 * (http://meets-ecommerce.de)
...
class Memcache extends Phalcon\Session\Adapter implements Phalcon\Session\AdapterInterface
{
	...
	{
		return $this->_getMemcacheInstance()
			->get($this->_getSessionId($sessionId), $this->getOption('lifetime'));
	}
	...
	{
		$this->_getMemcacheInstance()
			->save($this->_getSessionId($sessionId), $data, $this->getOption('lifetime'));
	}
	...
	{
		return $this->_getMemcacheInstance()
			->delete($this->_getSessionId($this->getId()));
	}
	...
	private function _getMemcacheInstance()
	{
		if($this->_memcacheInstance === null)
		{
			$this->_memcacheInstance = new Phalcon\Cache\Backend\Memcache(
				...
				array('host'=> $this->getOption('host'),'port'=> $this->getOption('port'),
