...
use \Phalcon\Session\Adapter, Phalcon\Session\AdapterInterface, Phalcon\Session\Exception;
...
class HandlerSocket extends Adapter implements AdapterInterface
{
	 ...
	 * 'dbname' => (string) : the database name of the mysql handlersocket
	 * server
	 * 'dbtable' => (string) : the table name of the mysql handlersocket server
	 */
	protected $_options = array('cookie_path'=> '/','cookie_domain'=> '','lifetime'=> 3600,
			'server'=> array('host'=> self::DEFAULT_HOST,'port'=> self::DEFAULT_PORT,'dbname'=> self::DEFAULT_DBNAME,
	...
	public function __construct($options = array())
	{
		...
		/*
		 * //set some important session vars ini_set('session.auto_start', 0);
		 * ini_set('session.gc_probability', 1); ini_set('session.gc_divisor',
		 * 100); ini_set('session.gc_maxlifetime', $this->_options['lifetime']);
		 * ini_set('session.referer_check', ''); ini_set('session.entropy_file',
		 * '/dev/urandom'); ini_set('session.entropy_length', 16);
		 * ini_set('session.use_cookies', 1);
		 * ini_set('session.use_only_cookies', 1);
		 * ini_set('session.use_trans_sid', 0); ini_set('session.hash_function',
		 * 1); ini_set('session.hash_bits_per_character', 5); //disable
		 * client/proxy caching session_cache_limiter('nocache'); //set the
		 * cookie parameters
		 * session_set_cookie_params($this->_options['lifetime'],
		 * $this->_options['cookie_path'], $this->_options['cookie_domain']);
		 * //name the session session_name('hs_sess'); //start it up
		 * //session_start();
		 */
	}
	...
	private function _init($options)
	{
		...
		if(! isset($this->_options['server'], $this->_options['server']['host'], $this->_options['server']['port'], 
			$this->_options['server']['dbname'], $this->_options['server']['dbtable']))
		{
			$this->_options['server'] = array('host'=> self::DEFAULT_HOST,'port'=> self::DEFAULT_PORT,
		...
		$this->_hs = new \HandlerSocket($this->_options['server']['host'], $this->_options['server']['port']);
		
		// open handlersocket index
		if(! ($this->_hs->openIndex($this->_hsIndex, $this->_options['server']['dbname'], 
			$this->_options['server']['dbtable'], \HandlerSocket::PRIMARY, self::DB_FIELDS)))
		{
			throw new Exception(
	...
	public function write($id, $data)
	{
		...
		{
			$this->_hs->executeInsert($this->_hsIndex, array($id,date('Y-m-d H:i:s'),$data));
		}
		else
		{
			$this->_hs->executeUpdate($this->_hsIndex, '=', array($id), array($id,date('Y-m-d H:i:s'),$data), 1, 0);
		}
	...
	public function gc($maxlifetime)
	{
		...
		$this->_hs->openIndex($index, $this->_options['server']['dbname'], $this->_options['server']['dbtable'], 
			self::DB_GC_INDEX, '');
