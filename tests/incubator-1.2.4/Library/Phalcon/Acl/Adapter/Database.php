 ...
 * this package in the file docs/LICENSE.txt. | | | | If you did not receive a
 * copy of the license and are unable to | | obtain it through the
 * world-wide-web, please send an email | | to license@phalconphp.com so we can
 * send you a copy immediately. |
 * +------------------------------------------------------------------------+ |
 * Authors: Andres Gutierrez <andres@phalconphp.com> | | Eduar Carvajal
 * <eduar@phalconphp.com> |
 * +------------------------------------------------------------------------+
 */
namespace Phalcon\Acl\Adapter;
use Phalcon\Acl, Phalcon\Acl\Adapter, Phalcon\Acl\AdapterInterface, Phalcon\Acl\Role, Phalcon\Acl\Resource, Phalcon\Acl\Exception;
...
class Database extends Adapter implements AdapterInterface
{
	 ...
	 *
	 * Example:
	 * <code>$acl->addRole(new Phalcon\Acl\Role('administrator'),
	 * 'consultor');</code>
	...
	public function addRole($role, $accessInherits = null)
	{
		...
		$exists = $this->_options['db']->fetchOne(
			'SELECT COUNT(*) FROM ' . $this->_options['roles'] . " WHERE name = ?", null, array($role->getName()));
		if(! $exists[0])
		{
			$this->_options['db']->execute('INSERT INTO ' . $this->_options['roles'] . " VALUES (?, ?)", 
				array($role->getName(),$role->getDescription()));
			$this->_options['db']->execute('INSERT INTO ' . $this->_options['accessList'] . " VALUES (?, ?, ?, ?)", 
	...
	public function addInherit($roleName, $roleToInherit)
	{
		...
		if(! $exists[0])
		{
			$this->_options['db']->execute('INSERT INTO ' . $this->_options['rolesInherits'] . " VALUES (?, ?)", 
	...
	{
		$exists = $this->_options['db']->fetchOne(
			'SELECT COUNT(*) FROM ' . $this->_options['roles'] . " WHERE name = ?", null, array($roleName));
	...
	{
		$exists = $this->_options['db']->fetchOne(
			'SELECT COUNT(*) FROM ' . $this->_options['resources'] . " WHERE name = ?", null, array($resourceName));
		return (bool)$exists[0];
	}
	 ...
	 * $acl->addResource(new Phalcon\Acl\Resource('customers'), array('create',
	 * 'search'));
	...
	public function addResource($resource, $accessList = null)
	{
		...
		$exists = $this->_options['db']->fetchOne(
			'SELECT COUNT(*) FROM ' . $this->_options['resources'] . " WHERE name = ?", null, 
			array($resource->getName()));
		if(! $exists[0])
		{
			$this->_options['db']->execute('INSERT INTO ' . $this->_options['resources'] . " VALUES (?, ?)", 
	...
	public function addResourceAccess($resourceName, $accessList)
	{
		...
		$sql = 'SELECT COUNT(*) FROM ' . $this->_options['resourcesAccesses'] .
		...
		else
		{
			...
			if(! $exists[0])
			{
				$this->_options['db']->execute('INSERT INTO ' . $this->_options['resourcesAccesses'] . " VALUES (?, ?)", 
	...
	protected function _insertOrUpdateAccess($roleName, $resourceName, $accessName, $action)
	{
		 ...
		 */
		$sql = 'SELECT COUNT(*) FROM ' . $this->_options['accessList'] .
		...
		{
			$sql = 'UPDATE ' . $this->_options['accessList'] .
	...
	public function isAllowed($role, $resource, $access)
	{
		 ...
		 */
		$sql = 'SELECT allowed FROM ' . $this->_options['accessList'] .
			 " WHERE roles_name = ? AND resources_name = ? AND access_name = ?";
