...
use Phalcon\Db, Phalcon\Cache\Backend, Phalcon\Cache\BackendInterface, Phalcon\Cache\Exception;
...
class Database extends Backend implements BackendInterface
{
	...
	public function save($keyName = null, $content = null, $lifetime = null, $stopBuffer = true)
	{
		...
		if(! $cache)
		{
			$options['db']->execute("INSERT INTO " . $options['table'] . " VALUES (?, ?, ?)", 
				array($keyName,$frontend->beforeStore($content),time()));
		}
		else
		{
			$options['db']->execute("UPDATE " . $options['table'] . " SET data = ?, lifetime = ? WHERE key_name = ?", 
				array($frontend->beforeStore($content),time(),$keyName));
		}
