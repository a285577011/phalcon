 ...
 *
 * Base class for \Phalcon\Mvc\Model\MetaData\Memcache and
 * \Phalcon\Mvc\Model\MetaData\Redis adapters.
 */
abstract class Base extends MetaData implements InjectionAwareInterface, MetaDataInterface
{
	 ...
	 * @var array
	 */
	protected static $defaults = array('lifetime'=> 8600,'prefix'=> '');
	...
	public function read($key)
	{
		return $this->getCacheBackend()
			->get($this->getId($key), $this->options['lifetime']);
	}
	...
	public function write($key, $data)
	{
		$this->getCacheBackend()
			->save($this->getId($key), $data, $this->options['lifetime']);
	}
	...
	{
		return (! empty($this->options['prefix']) > 0)? $this->options['prefix'] . '_' . $id: $id;
	}
