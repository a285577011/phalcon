...
class Firelogger extends \Phalcon\Logger\Formatter implements \Phalcon\Logger\FormatterInterface
{
	...
	private function pickle($var, $level = 0)
	{
		...
		elseif(is_array($var))
		{
			static $marker;
			if($marker === NULL)
				$marker = uniqid("\x00", TRUE); // detects
				                                                      // recursions
			if(isset($var[$marker]))
			{
				return '*RECURSION*';
	...
	private function extractTrace($trace)
	{
		...
		{
			// prevent notices about invalid indices, wasn't able to google
			// smart solution, PHP is dumb ass
			$frame += array('file'=> null,'line'=> null,'class'=> null,'type'=> null,'function'=> null,'object'=> null,
					'args'=> null);
			$t[] = array($frame['file'],$frame['line'],$frame['class'] . $frame['type'] . $frame['function'],
					$frame['object']);
			$f[] = $frame['args'];
		}
		;
	 ...
	 * @param int $order Optional. How many logs are stored in the stack
	 * already.
	 * @return mixed
	 */
	public function format($message, $type, $timestamp, $trace = NULL, $order = 0)
	{
		...
		$item = array('name'=> $this->_name,'args'=> array(),'level'=> $level,'timestamp'=> $timestamp,'order'=> $order, // PHP is really fast, timestamp has insufficient
		                   // resolution for log records ordering
				'time'=> gmdate('H:i:s', (int)$timestamp) . '.000','template'=> $message,'message'=> $message);
		if($this->style)
		{
		...
		if(isset($exception))
		{
			...
			$item['exc_info'] = array($exception->getMessage(),$exception->getFile(),$traceInfo[0]);
		...
		else
		{
			...
			if(isset($trace))
			{
				...
				$item['exc_info'] = array('','',$traceInfo[0]);
				$item['exc_frames'] = $traceInfo[1];
			}
