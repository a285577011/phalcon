...
use Phalcon\Logger\Exception, Phalcon\Logger\Formatter\Firelogger as FireloggerFormatter;
 ...
 * @license The BSD 3-Clause License {@link
...
class Firelogger extends \Phalcon\Logger\Adapter implements \Phalcon\Logger\AdapterInterface
{
	 ...
	 * Adapter options
	 *
	 * In addition to default options provided by Phalcon\Adapter, you may
	 * specify the following:
	 *
	 * (string) password Holds password which the client should send to turn
	 * Firelogger on. Leave empty if no password
	 ...
	 * _logs
	 *
	 * Storage for holding all messages until they are ready to be shipped to
	 * client.
	 *
	 * @var array
	 * @access protected
	...
	{
		$defaults = array('password'=> NULL,'checkVersion'=> TRUE,'traceable'=> FALSE);
		$this->_name = $name;
	 ...
	 *
	 * @param mixed $message Stuff to log. Can be of any type castable into a
	 * string (i.e. anything except for objects without __toString()
	 * implementation).
	 * @param int $type
	 * @param int $time
	 */
	public function logInternal($message, $type, $time)
	{
		...
		$log = $this->getFormatter()
			->format($message, $type, $time, $trace, count($this->_logs));
		$this->_logs[] = $log;
		...
		if(headers_sent($file, $line))
		{
			trigger_error(
				"Cannot send FireLogger headers after output has been sent" .
					 ($file? " (output started at $file:$line).": "."), \E_USER_WARNING);
			return;
		}
		$logs = $this->_logs;
		
		// final encoding
		$id = dechex(mt_rand(0, 0xFFFF)) . dechex(mt_rand(0, 0xFFFF)); // mt_rand
		                                                               // is not
		                                                               // working with
		                                                               // 0xFFFFFFFF
	 ...
	 * commit
	 *
	 * Encodes all collected messages into HTTP headers. This method is
	 * registered as a shutdown handler,
	 * so transactions will get committed even if you forget to commit them
	 * yourself.
	 *
	 ...
	 * checkPassword
	 *
	 * Checks client provided password to see if we should disable/enable the
	 * firelogger.
	 ...
	 * checkVersion
	 *
	 * Checks client version vs recommended version and logs a message if there
	 * is a mismatch. Does not
			...
			if($this->_clientVersion != $this->_recommendedClientVersion)
			{
				error_log(
					"FireLogger for PHP (v" . $this->_serverVersion .
						 ") works best with FireLogger extension of version " . $this->_recommendedClientVersion .
						 ". You are currently using extension v" . $this->_clientVersion .
						 ". Please install matching versions from http://firelogger.binaryage.com/ and https://github.com/phalcon/incubator/tree/master/Library/Phalcon/Logger");
				return FALSE;
			}
			return TRUE;
		}
