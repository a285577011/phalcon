...
class Extended extends Base
{
	...
	/**
	 * Jobs with smaller priority values will be scheduled before jobs with
	 * larger priorities.
	...
	/**
	 * Returns the result of command that wait the list in response from
	 * beanstalkd.
	 *
	 * @param string $cmd
	 * @return array null
		...
		if($nbBytes && ($nbBytes > 0))
		{
			...
			if(! preg_match('#^OK (\d+).*?#', $response, $matches))
			{
				throw new \RuntimeException(
					sprintf('Unhandled response: %s', $response));
			}
			...
			default:
				...
				else
				{
					...
					catch(\Exception $e)
					{
						...
						{
							$this->logger->warning(
								sprintf('Exception thrown when trying to delete job: %d — %s', $e->getCode(), 
									$e->getMessage()));
						}
					}
				}
